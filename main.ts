...
export default class MyPlugin extends Plugin {
	...
	override async onload() {
		...
		this.logger = createLogger('Open your mind', this.settings.devMode, this.settings.logLevel);

		this.logger.logPluginLoad();
		this.logger.debug('Settings loaded', this.settings);

		// This creates an icon in the left ribbon.
		const ribbonIconEl = this.addRibbonIcon('brain', 'Open your mind', (evt: MouseEvent) => {
		...
		this.addCommand({
			...
			callback: () => {
				...
				if (activeView) {
					...
					this.logger.debug('Word count calculated', { wordCount, contentLength: content.length });
...
*Happy note-taking!*`;
		...
		this.registerInterval(window.setInterval(() => this.logger.debug('Periodic interval tick'), 5 * 60 * 1000));
...
class SampleModal extends Modal {
	...
	constructor(app: App, logger: ReturnType<typeof createLogger>) {
		super(app);
		this.logger = logger;
	}

	override onOpen() {
		this.logger.debug('Sample modal opened');
		const {contentEl} = this;
		contentEl.setText('Woah! This is an enhanced modal.');
		
		// Add some interactive content
		const button = contentEl.createEl('button', {text: 'Click me!'});
		button.addEventListener('click', () => {
			this.logger.debug('Modal button clicked');
			new Notice('Button clicked in modal!');
			this.close();
		});
	}

	override onClose() {
		this.logger.debug('Sample modal closed');
		const {contentEl} = this;
		contentEl.empty();
	}
}
...
class SampleSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName('Dev mode')
			.setDesc('Enable detailed logging for debugging. Only enable when troubleshooting issues.')
			.addToggle(toggle => toggle
				...
				.onChange(async (value) => {
					...
					// Refresh the settings display to show/hide log level
					this.display();
				}));
